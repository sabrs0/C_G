...
def fill_area(stack, pause):
    ...
    while stack:
        ...
        #Идем вправо от затравочного пока не дойдем до границы или пикселя цвета заполнения
        ...
        for i in [1, -1]:
            ...
            while x <= rx:
                flag = 0
                #Выполняется если строчка не закрашена
...
for i in range(len(cfg.MODES)):
    mode_radios.append(tk.Radiobutton(data_frame, text=cfg.MODES[i], bg=cfg.MAIN_COLOUR,
                                      fg=cfg.ADD_COLOUR, variable=mode, value=i))
...
x_entry = tk.Entry(data_frame, font=("Consolas", 13), justify="center")
y_entry = tk.Entry(data_frame,font=("Consolas", 13), justify="center")
